...
sub run {
    ...
    my $hosts;
    if ( $hosts = $cache->get("hosts") ) {
        debugf "hit hosts cache";
    }
    else {
        debugf "miss hosts cache";
        $hosts = $api->call("host.get", {
            output => "extend",
        });
        $cache->set( screens => $hosts, $EXPIRES );
    }
    ...
    for my $s (@$hosts) {
        debugf "%s", ddf $s;
        $in->print($s->{hostid}, " ", encode_utf8($s->{name}), "\n");
    }
    $in->close;
    my $selected = <$out>;
    my ($hostid) = split / /, $selected, 2;

    exec "open", "${url}hosts.php?form=update&hostid=$hostid";
